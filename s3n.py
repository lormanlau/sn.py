...
corpus_name = "combine_general_corpus"
...
# printLines(os.path.join(corpus, "SW_EpisodeIV.txt"))
...
#                 if i == 2 and len(pair) < 2:
#                     pair.append(values[i].rstrip())
#                 elif i == 2:
...
# lines = {}
# conversations = []
# MOVIE_LINES_FIELDS = ["lineID", "character", "text"]
# MOVIE_CONVERSATIONS_FIELDS = ["character1ID", "character2ID", "movieID", "utteranceIDs"]

# # Load lines and process conversations
# print("\nProcessing corpus...")
# lines = loadLines(os.path.join(corpus, "SW_EpisodeIV.txt"), MOVIE_LINES_FIELDS)
...
# Configure models
model_name = 'combine_general_model'
# attn_model = 'dot'
attn_model = 'general'
#attn_model = 'concat'
hidden_size = 500
...
def sms():
  ...
  input_sentence = normalizeString(msg)
  res = MessagingResponse()
  try:
    output_words = evaluate(encoder, decoder, searcher, voc, input_sentence)
    # Format and print response sentence
    output_words[:] = [x for x in output_words if not (x == 'EOS' or x == 'PAD')]
    output = cleanupText(' '.join(output_words))
    res.message(output)
    return str(res)
  except:
    res.message('Sorry I didn t understand that')
    return str(res)
